...
class Field:
    def __init__(self, snakes: list[Snake], dirs: list[int], size: tuple = (40, 40)):
        if len(snakes) == len(dirs):
            self.snakes = {snakes[ind].id: [snakes[ind], dirs[ind]] for ind in range(len(snakes))}
            self.size = size
            self.apple = None
            self.make_apple()
    ...
    def move_snake(self, snake_id: int, dir: int):
        ...
        if self.snakes[snake_id][1] != restr[dir]:
            ...
            appl_eat = ((self.snakes[snake_id][0].predict(dir)[0] % self.size[0],
                         self.snakes[snake_id][0].predict(dir)[1] % self.size[1]) == self.apple)
            self.snakes[snake_id][0].move(dir, appl_eat)
